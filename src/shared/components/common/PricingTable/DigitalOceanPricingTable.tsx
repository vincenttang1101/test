...
import { DigitalOceanSizeDetail } from '@/shared/types/products.type'
import { cn, formatPrice } from '@/shared/utils/cores'

import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '../../cores/ui/select'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '../../cores/ui/tabs'
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '../../cores/ui/tooltip'
...
const productHeaderList: {
  ...
  },
  {
    column: 'Processor',
    isSortable: true,
    sortFunction: (a, b) => a.processor - b.processor,
    icon: (
      <svg xmlns='http://www.w3.org/2000/svg' width='25' height='24' viewBox='0 0 25 24' fill='none'>
        <path
  ...
  },
  {
    column: 'Storage',
    isSortable: true,
    sortFunction: (a, b) => a.storage - b.storage,
    icon: (
      <svg xmlns='http://www.w3.org/2000/svg' width='25' height='24' viewBox='0 0 25 24' fill='none'>
        <path
  ...
  },
  {
    ...
    isSortable: true,
    sortFunction: (a, b) => a.bandwidth - b.bandwidth,
    icon: (
      <svg xmlns='http://www.w3.org/2000/svg' width='25' height='24' viewBox='0 0 25 24' fill='none'>
        <path
...
type SizeTypeValue = 'Basic' | 'General Purpose' | 'CPU-Optimized' | 'Memory-Optimized' | 'Storage-Optimized'
...
const SizeTypes = [
  {
    ...
    value: 'Basic',
    description:
      'Máy ảo cơ bản có sự kết hợp giữa bộ nhớ và tài nguyên điện toán. Phù hợp nhất cho các dự án nhỏ có thể xử lý các mức hiệu suất CPU khác nhau, như blog, ứng dụng web và môi trường phát triển/thử nghiệm.'
  },
  {
    label: 'General Purpose',
    value: 'General Purpose',
    description:
      'Máy ảo hiệu suất cao với bộ nhớ cân bằng tốt và siêu luồng chuyên dụng từ bộ xử lý Intel tốt nhất. Một lựa chọn tuyệt vời cho nhiều khối lượng công việc sản xuất, phổ biến, như lưu trữ ứng dụng web, trang web thương mại điện tử, cơ sở dữ liệu cỡ trung bình và ứng dụng doanh nghiệp.'
  },
  {
    label: 'CPU-Optimized',
    value: 'CPU-Optimized',
    description:
      'Máy ảo được tối ưu hóa điện toán với các siêu luồng chuyên dụng từ bộ xử lý Intel tốt nhất. Phù hợp nhất cho các ứng dụng sử dụng nhiều CPU như CI/CD, mã hóa và chuyển mã video, học máy, phân phát quảng cáo, xử lý hàng loạt cũng như các máy chủ ứng dụng và web ngoại vi đang hoạt động. Đối với khối lượng công việc AI/ML, chẳng hạn như huấn luyện (Training) hoặc dự đoán (Inferencing).'
  },
  {
    label: 'Memory-Optimized',
    value: 'Memory-Optimized',
    description:
      'Máy ảo giàu bộ nhớ với bộ lưu trữ NVMe siêu nhanh, RAM 8GB cho mỗi vCPU và siêu luồng chuyên dụng từ bộ xử lý Intel tốt nhất trong phân khúc. Lý tưởng cho các ứng dụng sử dụng nhiều RAM như cơ sở dữ liệu hiệu suất cao, bộ nhớ đệm trong bộ nhớ ở quy mô web và xử lý dữ liệu lớn theo thời gian thực.'
  },
  {
    label: 'Storage-Optimized',
    value: 'Storage-Optimized',
    description:
      'Các Droplet có dung lượng lưu trữ NVMe siêu nhanh và lớn, phù hợp với cơ sở dữ liệu NoSQL lớn (ví dụ: MongoDB, Elaticsearch), cơ sở dữ liệu chuỗi thời gian (time series database) và các data warehouses khác.'
  }
]

const convertRam = (ram: number) => {
  if (ram < 1024) {
    return ram + ' MB'
  }
  return ram / 1024 + ' GB'
...
const convertData = (digitalOceanData: DigitalOceanData, sizeType: SizeTypeValue) => {
  return digitalOceanData[sizeType]
    .flatMap((data) => data.data)
    .sort((a, b) => a.price_monthly - b.price_monthly)
    .map((product) => {
      return {
        plan: product.slug,
        processor: product.vcpus,
        ram: product.memory,
        storage: product.disk,
        bandwidth: product.transfer,
        monthPrice: product.price_monthly,
        yearPrice: product.price_monthly * 12
      }
    })
}

export const DigitalOceanPricingTable = ({ digitalOceanData }: { digitalOceanData: DigitalOceanData }) => {
  const [sizeType, setSizeType] = useState<SizeTypeValue>('Basic')

  const digitalOceanProducts: Product[] = useMemo(() => {
    return convertData(digitalOceanData, sizeType)
  }, [digitalOceanData, sizeType])

  const [periodPrice, setPeriodPrice] = useState('monthly')
  const [products, setProducts] = useState<Product[]>(convertData(digitalOceanData, 'Basic'))
  ...
  useEffect(() => {
    if (sortColumn.column) {
      const sortedProducts = [...digitalOceanProducts].sort((a, b) => {
      ...
      setProducts(digitalOceanProducts)
    }
  }, [sizeType, digitalOceanProducts, sortColumn])

  return (
    <>
      <div className=''>
        <Tabs value={sizeType} onValueChange={(value) => setSizeType(value as SizeTypeValue)} className='mb-[10px]'>
          <TabsList className='] grid h-[unset] w-[900px] grid-cols-5 rounded-[94px] border border-[#eaebf9] bg-transparent p-0 text-xl'>
            {SizeTypes.map(({ value, description, label }) => (
              <TabsTrigger
                key={value}
                className='rounded-[94px] py-[8px] text-[0.875rem] font-normal data-[state=active]:bg-[#eaebf9] data-[state=active]:text-[#2f39bf]'
                value={value}
              >
                {label}
                {sizeType === value && (
                    ...
                    <Tooltip>
                      <TooltipTrigger>
                        <svg
                          ...
                          fill='none'
                        >
                          <g clip-path='url(#clip0_788_5422)'>
                            <path
                              d='M7 14C8.38446 14 9.73784 13.5895 10.889 12.8203C12.0401 12.0511 12.9373 10.9579 13.4672 9.67878C13.997 8.3997 14.1356 6.99224 13.8655 5.63437C13.5954 4.2765 12.9287 3.02922 11.9497 2.05025C10.9708 1.07128 9.7235 0.4046 8.36563 0.134504C7.00776 -0.135593 5.6003 0.00303004 4.32121 0.532844C3.04213 1.06266 1.94888 1.95986 1.17971 3.11101C0.410539 4.26215 -4.98905e-06 5.61553 -5.11009e-06 7C0.00200221 8.8559 0.740144 10.6352 2.05246 11.9475C3.36478 13.2598 5.1441 13.998 7 14ZM7 1.16667C8.15372 1.16667 9.28154 1.50879 10.2408 2.14976C11.2001 2.79074 11.9478 3.70178 12.3893 4.76768C12.8308 5.83358 12.9463 7.00647 12.7212 8.13803C12.4962 9.26958 11.9406 10.309 11.1248 11.1248C10.309 11.9406 9.26958 12.4962 8.13802 12.7212C7.00647 12.9463 5.83358 12.8308 4.76767 12.3893C3.70177 11.9478 2.79073 11.2001 2.14976 10.2408C1.50878 9.28154 1.16666 8.15372 1.16666 7C1.16836 5.45342 1.78349 3.97068 2.87708 2.87709C3.97068 1.78349 5.45342 1.16836 7 1.16667Z'
                              fill='#8993A4'
                            ></path>
                            <path
                              d='M7.00065 11.0835C7.15536 11.0835 7.30373 11.022 7.41313 10.9126C7.52253 10.8032 7.58398 10.6549 7.58398 10.5002L7.58398 5.8335C7.58398 5.67879 7.52253 5.53041 7.41313 5.42102C7.30373 5.31162 7.15536 5.25016 7.00065 5.25016C6.84594 5.25016 6.69757 5.31162 6.58817 5.42102C6.47878 5.53041 6.41732 5.67879 6.41732 5.8335L6.41732 10.5002C6.41732 10.6549 6.47878 10.8032 6.58817 10.9126C6.69757 11.022 6.84594 11.0835 7.00065 11.0835Z'
                              fill='#8993A4'
                            ></path>
                            <path
                              d='M6.41732 3.50016C6.41732 3.82233 6.67849 4.0835 7.00065 4.0835C7.32282 4.0835 7.58398 3.82233 7.58398 3.50016C7.58398 3.178 7.32282 2.91683 7.00065 2.91683C6.67849 2.91683 6.41732 3.178 6.41732 3.50016Z'
                              fill='#8993A4'
                            ></path>
                          </g>
                          <defs>
                            <clipPath id='clip0_788_5422'>
                              <rect
                                width='14'
                                height='14'
                                fill='white'
                      ...
                      <TooltipContent className='w-full whitespace-pre-wrap bg-white text-2xl text-black'>
                        {description}
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
          ...
          <TabsContent value={sizeType} className='mt-[20px]'>
              ...
              <ul className='price_header mx-auto my-0 flex w-full items-center justify-start pr-0'>
                ...
                <li className='flex w-[15%] justify-end'>
                  <Select value={periodPrice} onValueChange={setPeriodPrice}>
                    <SelectTrigger className='max-w-[80px] rounded-[44px] border border-solid border-[#c1c7d099] px-[9px] py-[5px] text-[12px] leading-[100%] text-[#253858]'>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
            ...
            <div className='priceWrapper mt-[25px] flex h-[516px] flex-col overflow-y-scroll px-[20px] max-lg:min-w-[939px]'>
              {products.map((product) => (
                ...
                >
                  <li>{product.plan}</li>
                  <li>{convertRam(product.ram)}</li>
                  <li>{product.processor} Core</li>
                  <li>{product.storage} GB</li>
                  <li>{product.bandwidth} TB</li>
                  <li className='flex items-baseline'>
                    {periodPrice === 'monthly' ? (
                      <>
                        <div className='mr-[4px] text-[1.5rem] font-semibold text-[#2f39bf]'>
                          {formatPrice(product.monthPrice * 25500, 'vi')}
                        </div>
                      ...
                      <>
                        <div className='mr-[4px] text-[1.5rem] font-semibold text-[#2f39bf]'>
                          {' '}
                          {formatPrice(product.yearPrice * 25500, 'vi')}
                        </div>
